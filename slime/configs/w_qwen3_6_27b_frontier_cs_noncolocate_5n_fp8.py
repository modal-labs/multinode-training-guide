"""Qwen3.6-27B Frontier-CS — FP8 ROLLOUT variant of ``w_qwen3_6_27b_frontier_cs_noncolocate_5n``.

Byte-identical to the baseline config except the SGLang rollout engines serve the
**official FP8 checkpoint** (``Qwen/Qwen3.6-27B-FP8``: blockwise e4m3 weights,
``weight_block_size [128,128]``, dynamic activation scales). Purpose: A/B the
rollout throughput (tok/s per GPU / per engine / per episode) against the BF16
baseline profiled on W&B run o22dad4t (~254 raw / ~209 trainable tok/s/GPU).

How FP8 rollout works here (see docs/en/advanced/low-precision.md):

* Megatron still trains in BF16 from ``ref_load`` (the existing
  ``Qwen3.6-27B_torch_dist`` volume checkpoint — NO new conversion needed).
* ``hf_checkpoint`` points at the FP8 HF checkpoint, so (a) SGLang serves FP8
  from step 0, and (b) at every weight resync slime reads the checkpoint's
  ``quantization_config`` and blockwise-requantizes the updated BF16 weights to
  FP8 (``quantize_params_fp8``) before pushing them into the engines.
* VERIFIED param-set match (2026-07-13): the official checkpoint's quantized set
  {q/k/v/o_proj, mlp gate/up/down, linear_attn in_proj_qkv/in_proj_z/out_proj,
  mtp attn+mlp} and its BF16 exclusions {in_proj_a/b, conv1d, A_log, dt_bias,
  norms, embed, lm_head, mtp.fc, the whole vision tower} line up 1:1 with what
  ``quantize_params_fp8`` quantizes for the qwen3_5 dense mapping — so resyncs
  keep the engine's param layout consistent. Do NOT swap in a locally-converted
  ``tools/convert_hf_to_fp8.py`` checkpoint without re-checking: that tool also
  quantizes the vision tower, which SGLang's qwen3_5 VLM loader does not accept.
* KV cache stays BF16 (``sglang_kv_cache_dtype`` unset) to isolate the
  weight-FP8 effect and keep rollout↔train logprob drift bounded.

KNOWN RISK — FP8 + gated-deltanet: the 35B-A3B FP8 attempt (2026-07-01, see
``qwen36_35b_a3b_agentic.py``) fixed the shared-expert block-quant blocker but
then hit STALLED episodes in FP8+GDN decode (examined=0 for 26+ min). This dense
27B is also a GDN hybrid. If episodes stall (rollout examined counts frozen,
requests hang), stop the app and record the failure — that IS the profile result.
...
Checkpointing + resume: same scheme as the baseline (per-launch save dir; Modal
auto-retry resumes; RESUME=<run tag> manually continues an earlier launch).

Before the first run (pull the FP8 checkpoint into the HF cache volume; the
Megatron torch_dist conversion is NOT needed — ref_load already exists):

    EXPERIMENT_CONFIG=w_qwen3_6_27b_frontier_cs_noncolocate_5n_fp8 \
...
_RUN_TAG = f"{os.environ.get('WANDB_GROUP') or 'qwen3.6-27b-frontier-cs-noncolocate-5n-fp8'}-{_LAUNCH_STAMP}"
...
class _Slime(SlimeConfig):
    ...
    # ── Model — FP8 ROLLOUT (the only functional change vs the baseline) ──────
    # SGLang serves the official blockwise-FP8 checkpoint; at every resync slime
    # requantizes the updated BF16 Megatron weights to FP8 using this
    # checkpoint's quantization_config (see module docstring). Training itself
    # stays BF16 off the SAME torch_dist ref_load as the baseline, so the two
    # runs differ only in rollout serving precision.
    hf_checkpoint = "Qwen/Qwen3.6-27B-FP8"
    # If the torch_dist conversion is ever re-run, it must read the BF16 repo —
    # never the FP8 one (Megatron trains in BF16).
    megatron_conversion_hf_checkpoint = "Qwen/Qwen3.6-27B"
