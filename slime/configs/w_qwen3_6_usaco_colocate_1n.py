...
Delta on ``w_qwen3_6_swe_colocate_1n``: same model/parallelism/optimizer/
topology, only the task changes. Harbor rows (``env/harbor.py``) boot the task's
Dockerfile sandbox (all USACO tasks share one ``python:3.13-slim`` image -> one
cached build), mini-swe-agent writes ``solution.py``, in-sandbox
``tests/test.sh`` grades, reward read from ``/logs/verifier/reward.txt``.
``download_data`` sparse-clones harbor-datasets, converts into ``/data/usaco``,
re-roots ``task_path`` under ``ASYNC_RL_TASK_ROOT=/data``, then pulls the
usaco_50 + openthoughts_tblite eval slices.

Eval: usaco_50 (in-distribution held-out) + openthoughts_tblite (held-out transfer).
...
# Inherit the colocated single-node Qwen3.6 SWE config (model/parallelism/
# optimizer/engine + modal infra); HF_EVAL_REPO is the published eval bundle.
...
class _Slime(_SweSlime):
    ...
    def download_data(self) -> None:
        """Stage the run's data: harbor USACO train set + eval slices.

        Sparse-clone harbor-datasets (usaco subtree), convert into /data/usaco,
        re-root task_path under /data, then pull the usaco_50 + openthoughts_tblite
        eval slices (eval-pinned copies win on the usaco overlap).
        """
        import json
        import subprocess
        import sys
