...
earlier (e.g. 24h-expired) run, relaunch with RESUME=<its run tag>:
    ...
    EXPERIMENT_CONFIG=frontier_cs.w_qwen3_6_27b_frontier_cs_noncolocate_5n \
...
class _Slime(SlimeConfig):
    ...
    async_mode = True            # train_async.py: trainer consumes batch N while pool generates N+1
    rollout_max_staleness = 4    # caps in-flight pool at 4 × rollout_batch_size = 128 groups
    rollout_function_path = "slime.rollout.fully_async_rollout.generate_rollout_fully_async"
    use_fault_tolerance = True   # a dead engine can't wedge the pool
    sglang_server_concurrency = 64   # per-engine cap; × 16 engines = 1024 concurrent episodes
    no_check_for_nan_in_loss_and_grad = True  # skip occasional NaN-grad step instead of crashing
    ...
    custom_config_path = {
        ...
        "agentic_exec_timeout": 60,  # per-command sandbox exec
    ...
    # ── Eval ──────────────────────────────────────────────────────────────────
    eval_interval = None
    skip_eval_before_train = True   # already set
